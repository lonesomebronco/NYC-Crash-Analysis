...
weekday_counts = borough_data['CRASH DATE'].dt.day_name().value_counts()

# Initializing a new matplotlib figure and axis for plotting
fig_hour_line, ax_hour_line = plt.subplots(figsize=(10, 6))

# Counting the number of accidents for each hour of the day
hour_of_day_counts = borough_data['CRASH DATE'].dt.hour.value_counts().sort_index()
...
print("Most accidents in 2019 occurred at:", f"{most_accidents_hour_2019}:00 hours")
print("Most accidents in 2020 occurred at:", f"{most_accidents_hour_2020}:00 hours")

# Creating a line plot with 12 points for the top 12 days
...
ax_top_12_days.invert_yaxis()  # Invert the y-axis to show the highest rank at the top
plt.tight_layout()
...
# Finding the day of the week with the most accidents
most_accidents_weekday = weekday_counts.idxmax()
print("5. Day of the Week with Most Accidents:", most_accidents_weekday)
...
july_2019_data = borough_data[(borough_data['CRASH DATE'].dt.year == 2019) & (borough_data['CRASH DATE'].dt.month == 7)]
july_2020_data = borough_data[(borough_data['CRASH DATE'].dt.year == 2020) & (borough_data['CRASH DATE'].dt.month == 7)]

# Initializing a new matplotlib figure and axis for plotting
fig_july_diff, ax_july_diff = plt.subplots(figsize=(12, 6))

# Plotting a bar chart to compare July 2019 and July 2020
sns.barplot(x='CRASH DATE', y='TOTAL PERSONS AFFECTED', hue=borough_data['CRASH DATE'].dt.year, data=borough_data, ax=ax_july_diff)

# Setting title and labels for the plot
ax_july_diff.set_title('Comparison of July 2019 vs. July 2020')
ax_july_diff.set_ylabel('Number of Persons Affected')
ax_july_diff.yaxis.set_major_locator(plt.MaxNLocator(integer=True))
ax_july_diff.legend() 
plt.tight_layout()
...
# Initializing a new matplotlib figure for plotting
plt.figure(figsize=(10, 6))

# Plotting a histogram for the distribution of 'NUMBER OF PERSONS INJURED'
sns.histplot(borough_data['NUMBER OF PERSONS INJURED'], bins=20, kde=True)

# Setting title and labels for the histogram
plt.title('Distribution of Number of Persons Injured in Accidents')
plt.xlabel('Number of Persons Injured')
plt.ylabel('Frequency')
plt.tight_layout()
plt.savefig('Distribution_of_no_of_persons_injured_in_accidents.png')
...
print(f"6. Hour of the Day with Most Accidents: {most_accidents_hour_12hr} {ampm}")
...
# Creating a new column 'Hour_of_Week' representing the hour of the week
df['Hour_of_Week'] = df['CRASH DATE'].dt.dayofweek * 24 + df['CRASH TIME'].dt.hour

# Finding the most common hour of the week for accidents
most_common_hour_of_week = df['Hour_of_Week'].mode().iloc[0]
most_common_hour_of_week_12hr = most_common_hour_of_week % 12 if most_common_hour_of_week % 12 != 0 else 12
ampm = 'AM' if most_common_hour_of_week < 12 else 'PM'
print("9. Hour of the Week Most Common for Accidents (12hr format):", f"{most_common_hour_of_week_12hr} {ampm}")
...
# Finding the top 10 worst days of the year based on the sum of persons injured
top_10_days = df.groupby(df['CRASH DATE'].dt.date)['NUMBER OF PERSONS INJURED'].sum().nlargest(10)
print("12. 10 Worst Days of the Year:")
print(top_10_days)
...
print("13. Are Amazon Truck Crashes an Issue?", are_amazon_truck_crashes_an_issue)


# Plotting the most common type of crash
plt.figure(figsize=(10, 6))
